(function () {
  ...
  function initCurtain() {
    ...
    curtain.innerHTML =
      ...
      '<div class="curtain-logo">Shed&nbsp;<span class="accent">Cutz</span></div>';
  ...
  function initPaymentNotice() {
    ...
    /* FAB — toujours créé, CSS le cache sur desktop */
    ...
    /* FAB : apparaît après chargement (CSS le cache sur desktop) */
    setTimeout(function () {
      fab.classList.add('is-visible');
    }, 600);

    fab.addEventListener('click', function () {
      fab.style.cssText = 'transition:transform 0.25s ease,opacity 0.25s ease;transform:scale(0);opacity:0;';
      setTimeout(showNotice, 220);
    });

    /* Desktop : popup automatique après le rideau */
    ...
    notice.querySelector('.payment-notice-close').addEventListener('click', function () {
      hideNotice(function () {
        /* Remet le FAB visible (utile sur mobile) */
        fab.style.cssText = '';
        fab.classList.add('is-visible');
      });
    });
